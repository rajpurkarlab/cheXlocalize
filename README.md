...
**Optional flags**
* `--true_pos_only`: Default is `True`. If `True`, run evaluation only on the true positive slice of the dataset (CXRs that contain both predicted and ground-truth segmentations). If set to false, also include CXRs with predicted segmentation but without ground-truth segmentation, and CXRs with ground-truth segmentation but without predicted segmentation.
